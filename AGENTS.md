...
2. 批量、并行地准备相互独立的改动与验证命令。
3. 实施后立即运行相应验证：`pnpm check`、`pnpm test`、`pnpm build`（必要时本地 `pnpm preview` + 浏览器/HTTP 冒烟）。
4. 门禁失败不得绕过：回到改动处修复，直到全绿；确实无关的存量失败要单独记录到 `docs/build-log.md`，不得静默掩盖。
5. 提交采用 Conventional Commits（`feat:` / `fix:` / `docs:` / `test:` / `chore:` / `refactor:`），提交信息必须反映实际内容。
